...
Dựa trên góc độ chuyên gia hệ thống (Senior System Designer), kiến trúc dự án đã được tối ưu hóa để đáp ứng khả năng mở rộng (Scalability), tính toàn vẹn dữ liệu (Data Integrity), và bổ sung hệ thống quản lý trung tâm (Centralized Admin Management).

Cải tiến phần lõi bao gồm:

1. **Relational Database (SQL)**: Cấu trúc RDBMS được thêm vào nhằm quản lý User, phân quyền tài khoản (User/Admin), lưu trữ lịch sử hội thoại (Conversation & Message History) một cách tin cậy.
2. **Admin CMS Component**: Hệ thống quản trị chuyên biệt giám sát lưu lượng, đánh giá (Rating) và các hành vi xấu (Ban/Unban users).

Để tối ưu xuất PDF/Word, tất cả sơ đồ luồng (flowchart) được tiêu chuẩn hoá theo hướng từ trên xuống (TD - Top Down).

Dưới đây là 8 sơ đồ kiến trúc hoàn chỉnh biểu diễn từ tổng thể đến chi tiết:

---

## 1️⃣ Sơ đồ Kiến trúc Tổng thể (Overall System Architecture)
...
```mermaid
flowchart TD
  Client[Headless Drupal UI] -->|REST API| API[FastAPI Backend / API Gateway]
  
  API --> Auth[Auth Component]
  API --> Admin[Admin Component]
  API --> Core[AI Core Component]
  
  Auth --> RDB[(Relational DB\nPostgreSQL/MySQL\nUsers/Sessions)]
  Admin --> RDB
  Core --> RDB
  
  Core --> ExtProcessor[Document Extractor & Processor]
  Core --> Embed[Embedding Model Component]
  Embed --> VDB[(Vector DB\nChromaDB\nDocument Embeddings)]
  
  Core --> LLM[LLM API / Qwen / Llama]
```

---

## 2️⃣ Sơ đồ Cơ sở Dữ liệu (Entity Relationship Diagram - ERD)
...
## 3️⃣ Luồng Người dùng Cơ bản (Main User Flow)
...
flowchart TD
  ...
  T --> Process[Prompt Engineering Module]
  F --> Document[Document RAG Module]
  
  Process --> Gen[LLM Summarization Generate]
  Document --> Gen
...
## 4️⃣ Pipeline cho Prompt Engineering (Text ngắn)

Kiến trúc hệ thống dành cho Request nhỏ không cần VectorDB. Tối ưu độ trễ (Latency).

```mermaid
flowchart TD
  A[Nhập Văn bản từ Headless Drupal] --> B[Gắn Context / Prompt Template]
  B --> C[AI Engine / LLM]
  C --> D[Văn bản Tóm tắt]
  D --> E[(Lưu Log Message vào CSDL SQL)]
  E --> F[Trả về Headless Drupal UI]
```

---

## 5️⃣ Pipeline cho RAG (Document dài)
...
flowchart TD
  ...
  K --> L[Trả về kết quả tới Headless Drupal UI]
```

---

## 6️⃣ Kiến trúc luồng Backend API (Backend Routing Structure)
...
```mermaid
flowchart TD
  UI[Headless Drupal UI Web/Mobile] --> API[FastAPI Application]
...
## 7️⃣ Luồng Lưu trữ và Tái tạo Lịch sử Hội thoại (Chat History Flow)
...
sequenceDiagram
  actor User
  participant Frontend as Headless Drupal UI
...
## 8️⃣ Luồng Quản trị Hệ thống (Admin Management Flow)
...
## 9️⃣ Luồng Phân tích Dữ liệu Hệ thống (PowerBI Analytics Flow)
